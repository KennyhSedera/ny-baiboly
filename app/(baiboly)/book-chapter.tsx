...
const ScreenBookChapter = () => {
  ...
  const { books, verses, color } = useApp();
    ...
    <ThemedView style={styles.container}>
      <View style={[styles.header, { backgroundColor: color?.bg, borderColor: color?.text }]}>
        <View style={styles.flexRow}>
          ...
          <Text style={[styles.headerTitle, { color: color?.text }]}>{book?.long_name}</Text>
        </View>
      </View>
      <ScrollView style={{ flex: 1 }} contentContainerStyle={[styles.booksContainer, styles.scrollContent]} showsVerticalScrollIndicator={false} >
        {chapters?.map((chapter) => (
          <Pressable
            ...
            style={[styles.bookCard, { width: "19%", borderColor: `${book?.book_color}30`, backgroundColor: `${book?.book_color}30` }]}
          >
